...
{
    private string savePath = "Saves/save.txt";
    ...
    // /// <summary>
    // /// Initilizes an base Neural Network to be Loaded (Specific Use)
    // /// </summary>
    ...
    public void Save()
    {
        Debug.Log("Saving NeuralNetwork");
        
        using (var stream = File.OpenWrite(savePath))
        using (var writer = new BinaryWriter(stream))
        {
            writer.Write((short)layers.Length);
            foreach (var layer in layers)
            {
                writer.Write(layer);
            }
            writer.Write((short)weights.Length);
            foreach (var weight0 in weights)
            {
                writer.Write((short)weight0.Length);
                foreach (var weight1 in weight0)
                {
                    writer.Write((short)weight1.Length);
                    foreach (var weight2 in weight1)
                    {
    ...
    {
        //Debug.Log("Loading NeuralNetwork");
        NeuralNetwork tempNet;
        ...
        using (var reader = new BinaryReader(stream))
        {
            ...
            tempNet = new NeuralNetwork(layers);

            int weightsLen = reader.ReadInt16();
            for (int i = 0; i < weightsLen; i++)
            {
                int weights1Len = reader.ReadInt16();
                for (int j = 0; j < weights1Len; j++)
                {
                    int weights2Len = reader.ReadInt16();
                    for (int k = 0; k < weights2Len; k++)
                    {
                        tempNet.weights[i][j][k] = reader.ReadSingle();
                    }
                }
            }
        }
